...
BEGIN
    IF INSERTING THEN
        ...
        VALUES ('ORDERS',
                ...
                'customer_id=' || :NEW.customer_id || ', order_date=' ||
                TO_CHAR(:NEW.order_date, 'YYYY-MM-DD HH24:MI:SS') ||
    ...
    ELSIF UPDATING THEN
        ...
        VALUES ('ORDERS',
                ...
                'customer_id=' || :OLD.customer_id || ', order_date=' ||
                TO_CHAR(:OLD.order_date, 'YYYY-MM-DD HH24:MI:SS') ||
...
CREATE OR REPLACE PACKAGE BODY report_pkg AS
    ...
    BEGIN
        ...
        v_report := v_report || '<tr><td>CUSTOMERS</td><td>' || v_ins_customers || '</td><td>' || v_upd_customers ||
                    '</td><td>' || v_del_customers || '</td></tr>';
        v_report :=
                v_report || '<tr><td>ORDERS</td><td>' || v_ins_orders || '</td><td>' || v_upd_orders || '</td><td>' ||
                v_del_orders || '</td></tr>';
        v_report := v_report || '<tr><td>ORDER_ITEMS</td><td>' || v_ins_items || '</td><td>' || v_upd_items ||
                    '</td><td>' || v_del_items || '</td></tr>';
...
BEGIN
    ...
    FROM (SELECT a.*, ROW_NUMBER() OVER (PARTITION BY pk_value ORDER BY change_time DESC) rn
          FROM audit_log a
          WHERE table_name = 'CUSTOMERS'
...
VALUES (9, 'Заказ от Alice', 150);
INSERT INTO orders (customer_id, comment_text, amount)
VALUES (10, 'Заказ от Bob', 250);
COMMIT;
...
INSERT INTO orders (customer_id, comment_text, amount)
VALUES (9, 'Дополнительный заказ', 300);
...
BEGIN
  rollback_pkg.rollback(TIMESTAMP '2025-04-03 18:14:30');
END;
